...
// Está função esta validando os dados de entrada para login.
...
const validateBodyLogin = (body) => {
  const userSchema = Joi.object({
    email: Joi.string().email().required().messages({
      ...
      'string.base': '400|The "email" field must be a string',
    }),
    password: Joi.string().min(6).required().messages({
      ...
      'string.base': '400|The "password" field must be a string',
    }),
  });
...
// Está função esta validando os dados de cadastro de usuário.
// Caso surja alguma incompatibilidade, retorno o erro personalizado no formato 'status|message'.
